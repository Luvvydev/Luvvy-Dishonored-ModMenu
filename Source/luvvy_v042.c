...
static GFxValue g_OriginalText[8];
static GFxValue g_OriginalCallbacks[8];
static void *g_OriginalMovie=0;
static BOOL g_HaveOriginals=FALSE;
...
static void CaptureOriginals(void*movie,GFxValue*pauseMenuMc){u32 i;if(!movie||!pauseMenuMc)return;if(g_HaveOriginals&&g_OriginalMovie==movie)return;g_OriginalMovie=movie;g_HaveOriginals=TRUE;for(i=0;i<8;++i){GetMovieVariable(movie,&g_OriginalText[i],g_TextPaths[i]);ValueGetMember(pauseMenuMc,g_Callbacks[i],&g_OriginalCallbacks[i]);}ValueSetMember(pauseMenuMc,"_LuvvyOriginalResume",&g_OriginalCallbacks[0]);}
static void RestoreOriginals(void*movie,GFxValue*pauseMenuMc){u32 i;if(!g_HaveOriginals||g_OriginalMovie!=movie)return;for(i=0;i<8;++i){SetMovieVariable(movie,g_TextPaths[i],&g_OriginalText[i]);ValueSetMember(pauseMenuMc,g_Callbacks[i],&g_OriginalCallbacks[i]);}ValueSetMember(pauseMenuMc,"_LuvvyOriginalResume",&g_OriginalCallbacks[0]);}
static void ResumeOriginal(FunctionParams*params){GFxValue pauseMenuMc;if(!params||!params->movie)return;if(!GetPauseMenuMC(params->movie,&pauseMenuMc))return;ValueInvoke(&pauseMenuMc,0,"_LuvvyOriginalResume",0,0);}
static void SetRow(void*movie,GFxValue*pauseMenuMc,u32 row,wchar_t*label,u32 action){GFxValue text,callback;if(row>=8)return;SetString(&text,label);SetMovieVariable(movie,g_TextPaths[row],&text);CreateFunction(movie,&callback,action);ValueSetMember(pauseMenuMc,g_Callbacks[row],&callback);}
...
static void __thiscall HandlerCall(FunctionHandler*self,FunctionParams*params){u32 action;(void)self;if(!params)return;action=(u32)params->userData;
 if(action>=ACTION_HEALTH&&action<=ACTION_POTIONS){TogglePatch(action-1);ResumeOriginal(params);return;}
 if(action==ACTION_OPEN_CHEATS){g_CheatPage=TRUE;ResumeOriginal(params);return;}
 if(action==ACTION_BACK_NORMAL){g_CheatPage=FALSE;ResumeOriginal(params);return;}
 if(action==ACTION_RESUME){ResumeOriginal(params);return;}
}
...
static void __cdecl Luvvy_OnPauseMenu(void*pauseMenu){void*disMovie;void*movie;GFxValue pauseMenuMc;if(!pauseMenu)return;if(*((u8*)pauseMenu+0x1F8)==2)return;disMovie=*(void**)((u8*)pauseMenu+0x38);if(!disMovie)return;movie=*(void**)((u8*)disMovie+0x34);if(!movie)return;if(!GetPauseMenuMC(movie,&pauseMenuMc))return;
 CaptureOriginals(movie,&pauseMenuMc);
 ...
 SetRow(movie,&pauseMenuMc,5,L"BACK TO NORMAL MENU",ACTION_BACK_NORMAL);
 SetRow(movie,&pauseMenuMc,6,L"CLICKING A CHEAT APPLIES + RESUMES",ACTION_RESUME);

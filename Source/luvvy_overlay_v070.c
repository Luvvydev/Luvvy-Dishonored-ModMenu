...
/* -------------------- Blink extras --------------------
   The verified game layout exposes the current player pawn at 0x01452DE8.
   The Blink active-power component is the pointer at pawn + 0xAA8.
   A public Dishonored trainer documents the live Blink cooldown timer at
   component + 0x138. Keeping that value at 1.0 removes Blink's reuse wait.

   Range fields are config-backed members on DishonoredActivePowerComponent_Blink.
   We locate the stock 1400 / 600 max-distance floats inside the live Blink
   component instead of hardcoding member offsets that may vary by retail build.
*/
static BOOL g_NoBlinkCooldown=FALSE;
static BOOL g_LongBlink=FALSE;
static float *g_BlinkHoriz=0;
static float *g_BlinkVert=0;
static float g_BlinkHorizOriginal=1400.0f;
static float g_BlinkVertOriginal=600.0f;
...
static BOOL FloatNear(float a,float b){
    float d=a-b;if(d<0)d=-d;return d<0.01f;
}
static BOOL LocateBlinkRangeFields(void){
    u8 *blink=GetBlinkComponent();
    u32 i,j;
    float *h=0,*v=0;
    if(!blink)return FALSE;

    /* Search only the Blink object's first 0x500 bytes.
       Require both stock max-distance values to be near each other. */
    for(i=0;i<0x500;i+=4){
        float *p=(float*)(blink+i);
        if(FloatNear(*p,1400.0f) || FloatNear(*p,2800.0f)){
            for(j=(i>0x80?i-0x80:0);j<i+0x100 && j<0x500;j+=4){
                float *q=(float*)(blink+j);
                if(FloatNear(*q,600.0f) || FloatNear(*q,1200.0f)){
                    h=p;v=q;break;
                }
            }
        }
...
static void ToggleLongBlink(void){
    if(!g_BlinkHoriz || !g_BlinkVert){
        if(!LocateBlinkRangeFields())return;
    }
    if(!g_LongBlink){
        /* Capture whatever the current build/config is using before changing it. */
        g_BlinkHorizOriginal=*g_BlinkHoriz;
        g_BlinkVertOriginal=*g_BlinkVert;
        *g_BlinkHoriz=g_BlinkHorizOriginal*2.0f;
        *g_BlinkVert=g_BlinkVertOriginal*2.0f;
        g_LongBlink=TRUE;
    }else{
        *g_BlinkHoriz=g_BlinkHorizOriginal;
        *g_BlinkVert=g_BlinkVertOriginal;
        g_LongBlink=FALSE;
    }
}
...
static const WCHAR *GetLabel(int i){
    ...
    return g_LongBlink?L_BLINKR_ON:L_BLINKR_OFF;
}
...
static LRESULT __stdcall OverlayWndProc(HWND hwnd,UINT msg,WPARAM wp,LPARAM lp){
    ...
    if(msg==WM_PAINT){
        ...
        r.left=0;r.top=0;r.right=440;r.bottom=380;
        ...
        pTextOutW(dc,22,326,HELP_TEXT,WLen(HELP_TEXT));
        pEndPaint(hwnd,&ps);
        return 0;
    }
...
static BOOL EnsureOverlay(void){
    ...
    g_Overlay=pCreateWindowExW(
        ...
        100,100,440,380,0,0,g_Instance,0
    );
...
static void PositionOverlay(void){
    ...
    if(fg&&pGetWindowRect(fg,&rr)){
        ...
        y=rr.top+((rr.bottom-rr.top)-380)/2;
        pSetWindowPos(g_Overlay,HWND_TOPMOST,x,y,440,380,SWP_NOACTIVATE|SWP_SHOWWINDOW);
    }
}
...
static void OverlayTick(void *gfx){
    ...
    if(KeyPressed(VK_UP)){g_Selected=(g_Selected+6)%7;Redraw();}
    if(KeyPressed(VK_DOWN)){g_Selected=(g_Selected+1)%7;Redraw();}
    if(KeyPressed(VK_RETURN)){
        if(g_Selected<5)TogglePatch((u32)g_Selected);
        else if(g_Selected==5)g_NoBlinkCooldown=!g_NoBlinkCooldown;
        else ToggleLongBlink();
        Redraw();
    }
    ...
    if(KeyPressed(VK_6)){g_NoBlinkCooldown=!g_NoBlinkCooldown;g_Selected=5;Redraw();}
    if(KeyPressed(VK_7)){ToggleLongBlink();g_Selected=6;Redraw();}
}

...
static void __thiscall HandlerCall(FunctionHandler*self,FunctionParams*params){u32 action;(void)self;if(!params)return;action=(u32)params->userData;
 /* IMPORTANT: never close/reopen the Scaleform pause menu from inside a
    custom callback. Doing that re-enters the menu while ActionScript is still
    dispatching the button click, which is what caused the intermittent blank
    submenu on the second selection. */
 if(action>=ACTION_HEALTH&&action<=ACTION_POTIONS){TogglePatch(action-1);return;}
 if(action==ACTION_OPEN_CHEATS){g_CheatPage=TRUE;return;}
 if(action==ACTION_BACK_NORMAL){g_CheatPage=FALSE;return;}
 if(action==ACTION_RESUME){return;}
}
...
static void __cdecl Luvvy_OnPauseMenu(void*pauseMenu){void*disMovie;void*movie;GFxValue pauseMenuMc;if(!pauseMenu)return;if(*((u8*)pauseMenu+0x1F8)==2)return;disMovie=*(void**)((u8*)pauseMenu+0x38);if(!disMovie)return;movie=*(void**)((u8*)disMovie+0x34);if(!movie)return;if(!GetPauseMenuMC(movie,&pauseMenuMc))return;
 ...
 /* Row 7 stays original. Press Esc uses Dishonored's untouched native
    pause-menu back/resume handling. */
}
